...
class Transformer(nn.Module):
    def __init__(self, vocab_size, emb_dim, n_heads, context_len, n_block, dropout, device, 
                n_experts, K, alpha, m, d_latent, d_rope, n_predicted_tokens):
        super().__init__()
        self.emb = nn.Embedding(vocab_size, emb_dim)
        self.trunk_blocks = nn.ModuleList(
            [DecoderBlock(emb_dim, n_heads, context_len, dropout, n_experts, K, m, d_latent, d_rope, device) for _ in range(n_block-1)]
        )
        self.leaf_blocks = nn.ModuleList(
            [DecoderBlock(emb_dim, n_heads, context_len, dropout, n_experts, K, m, d_latent, d_rope, device) for _ in range(n_predicted_tokens)]
        )
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('proj.weight'):
                torch.nn.init.normal_(p, mean=0.0, std=0.02/math.sqrt(2 * (n_block + n_predicted_tokens - 1))) # because of MTP
    ...
    def forward(self, x, y=None, training=False):
        ...
        if y is not None:
            ...
            z = x # variable names as in MTP paper
            d = z.detach() # to backprop until d (result of common blocks)
            ...
            aux_loss = self.alpha * aux_loss # for MoE
